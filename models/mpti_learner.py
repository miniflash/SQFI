...
from models.mpti_gfim import MPTI_gfim
...
class MPTILearner(object):
            ...
            if args.use_attention:
                self.optimizer = torch.optim.Adam(
                    [{'params': self.model.encoder.parameters(), 'lr': 0.0001},
                     ...
                     {'params': self.model.iteraction_learner.parameters()}], lr=args.lr)
            else:
                self.optimizer = torch.optim.Adam(
                    [{'params': self.model.encoder.parameters(), 'lr': 0.0001},
                     {'params': self.model.base_learner.parameters()},
    ...
    def train(self, data):
        ...
        query_logits, loss= self.model(support_x, support_y, query_x, query_y, aux_loss=True)
    ...
    def test(self, data):
        ...
        with torch.no_grad():
            logits, loss= self.model(support_x, support_y, query_x, query_y, aux_loss=False)
